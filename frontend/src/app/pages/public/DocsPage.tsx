...
const docContent: Record<string, { title: string; content: React.ReactNode }> = {
    ...
    content: (
      <div className="space-y-6">
        ...
        <div className="p-4 rounded-xl border border-accent/30 bg-accent/5">
          <p className="text-accent text-sm font-semibold mb-1">Quick tip</p>
          <p className="text-muted-foreground text-sm">
            Start with the Quick Start Guide if you want to be up and running in under 5 minutes.
          </p>
        </div>
        <h3 className="text-foreground font-semibold text-lg">What is CloudTask Pro?</h3>
        <p className="text-muted-foreground leading-relaxed">
        ...
        <h3 className="text-foreground font-semibold text-lg">Key concepts</h3>
  ...
  "quick-start": {
    ...
    content: (
      <div className="space-y-6">
        <p className="text-muted-foreground leading-relaxed">Get your workspace up and running in under 5 minutes.</p>
        {[
          ...
          <div key={step} className="flex gap-4">
            <div className="w-8 h-8 rounded-full bg-gradient-to-br from-accent to-[#8B5CF6] flex items-center justify-center text-foreground font-bold text-sm flex-shrink-0">
              {step}
            </div>
            <div>
  ...
  "api-overview": {
    ...
    content: (
      <div className="space-y-6">
        ...
        <h3 className="text-foreground font-semibold text-lg">Rate limits</h3>
        <p className="text-muted-foreground text-sm leading-relaxed">
          Free: 100 requests/min · Pro: 1,000 requests/min · Enterprise: Custom
        </p>
        <h3 className="text-foreground font-semibold text-lg">Response format</h3>
        <div className="rounded-xl border border-white/10 bg-black/40 overflow-hidden">
    ...
    content: (
      <div className="space-y-6">
        <div className="p-4 rounded-xl border border-[#F59E0B]/30 bg-[#F59E0B]/5">
          ...
          <p className="text-muted-foreground text-sm">SSO and SAML 2.0 are available on the Enterprise plan.</p>
        </div>
        <p className="text-muted-foreground leading-relaxed">
        ...
        <h3 className="text-foreground font-semibold text-lg">Setup steps</h3>
...
function DefaultDocContent({ title }: { title: string }) {
  return (
    <div className="space-y-6">
      <p className="text-muted-foreground leading-relaxed">
        This section covers <strong className="text-foreground">{title}</strong> — comprehensive documentation
      ...
      <div className="p-4 rounded-xl border border-white/10 bg-white/3 flex items-center gap-3">
        ...
        <span className="text-muted-foreground text-sm">
          ...
          <a href="mailto:support@cloudtaskpro.com" className="text-accent hover:underline">
        ...
        <div className="max-w-7xl mx-auto">
          ...
          <div className="relative max-w-md">
            ...
            <input
              ...
              className="w-full bg-white/5 border border-white/10 rounded-xl pl-10 pr-4 py-2.5 text-foreground text-sm placeholder-white/25 focus:outline-none focus:border-accent/50 transition-colors"
            />
          </div>
        </div>
      </div>
              ...
              <div key={id}>
                ...
                >
                  <div className="flex items-center gap-2.5">
                    ...
                    <span className="text-foreground/80 text-sm font-semibold">{label}</span>
                  </div>
                  <motion.span
                      ...
                      <button
                        ...
                        className={`w-full text-left px-3 py-1.5 rounded-lg text-sm transition-colors ${
                          activeItem === item.id
                            ...
                            : "text-muted-foreground hover:text-muted-foreground hover:bg-white/4"
                        }`}
                      >
                        {item.label}
                      </button>
